...
class InvertedPendulum:
    ...
    def update(self):
        ...
        frequency = 100
        amplitude = 1
        sinewave = amplitude * np.sin(2 * np.pi * frequency * time + heta)   
        #print(sinewave)
        ...
        for i in range(1000):
         self.force=self.force*sinewave[i]
         ...
         #acc_cart
         acc_c= np.append(acc_c,[self.x_d_dot])
         
         self.theta_d_dot = (self.g * sin(self.theta) - self.x_d_dot * cos(self.theta)) / self.length
         #pendulam acc
         acc_p= np.append(acc_p,[self.theta_d_dot])
         
         self.x_dot = self.x_dot + self.x_d_dot * self.dt
         #velocity_cart
         ve_c= np.append(ve_c,[self.x_dot])
         self.x = self.x + self.x_dot * self.dt
         
         self.theta_dot = self.theta_dot + self.theta_d_dot * self.dt
         #pendulam velocity
         ve_p= np.append(ve_p,[self.theta_dot])
         self.theta = self.theta + self.theta_dot * self.dt
         #dt=np.array([self.dt])
          
        #figure(figsize=(20, 6), dpi=80)
        timee= np.arange(0,1001,1)
        plt.title("cart _acceleration")
        plt.plot(timee,acc_c)
        plt.show()
        plt.title("pendulum _acceleration")
        plt.plot(timee,acc_p)
        plt.show()
        plt.title("pendulum _velocity")
        plt.plot(timee,ve_p)
        plt.show()
        plt.title("cart_velocity")
        plt.plot(timee,ve_c)
        plt.show()
